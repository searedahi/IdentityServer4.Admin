...
namespace Skoruba.IdentityServer4.Admin.UnitTests.Mocks
{
    public class PersistedGrantMock
    {
        ...
        public static Faker<PersistedGrant> PersistedGrantFaker(string key, Guid subjectId)
        {
            var persistedGrantFaker = new Faker<PersistedGrant>()
                ...
                .RuleFor(o => o.SubjectId, f => string.IsNullOrEmpty(subjectId.ToString()) ? Guid.NewGuid().ToString() : subjectId.ToString());

            return persistedGrantFaker;
        }

        public static PersistedGrant GenerateRandomPersistedGrant(string key, Guid subjectId)
        {
